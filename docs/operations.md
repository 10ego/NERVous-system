...
While a NERVous chain is active, a transient provider transport failure such as a WebSocket disconnect queues one visible automatic recovery nudge. The nudge steers Pi's native retry when enabled and drives a continuation when native retries are disabled or exhausted. It tells the agent to reconcile durable state and reuse committed results before replaying side effects, especially when the interrupted response contained unresolved tool calls. Recovery is bounded to one attempt until it succeeds or the user starts a new turn, preventing a failed recovery from looping. NERVous does not modify Pi's global `retry.enabled` setting; inactive session chains retain ordinary Pi behavior.
