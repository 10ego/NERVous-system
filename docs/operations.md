...
Reset moves every raw artifact in the active namespace—including backups, invalid clean-slate ledgers, and LION progress sidecars—to `<NERVOUS_STATE_ROOT>/<project>/.archive/...` (under `~/.pi/nervous` by default); it does not parse, migrate, or partially delete cross-referenced records. Component stores then recreate an empty namespace on their next write. Archives are retained for at least 30 days by default and archives past that age are pruned during a later reset. Set `NERVOUS_ARCHIVE_RETENTION_DAYS=0` before launching Pi to retain reset archives indefinitely.

For safety, reset waits for the main agent to become idle, refuses live component lock holders, and is unavailable while explicit component path overrides are active. Queued/running LION records—or an unreadable LION ledger whose liveness cannot be classified—require `force`; force is not process cancellation and must never be used while a worker may still be alive. Other named/branch contexts are not auto-expired; select the intended `NERVOUS_CONTEXT` before resetting one.
