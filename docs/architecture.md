...
The root package's always-loaded controller is also an activation boundary. On session start it removes the root suite's model tools from Pi's active tool set. Only exact user/RPC `/nervous` input grants a one-agent-run lease; completion restores the previous tool set, while a `tool_call` hook blocks unauthorized calls. Standalone component packages do not load this root controller and therefore retain their independent behavior.
