...
This runbook covers activation, normal operation, recovery, and emergency shutdown for the `nervous-system` npm release pipeline. GitHub tags, GitHub releases, workflow artifacts, and npm versions are immutable release records; never move, reuse, or replace them.
...
Generate or rotate the App private key, then upload it without putting it in a shell argument or repository file:

```bash
gh secret set NERV_OPS_PRIVATE_KEY \
  --repo 10ego/NERVous-system \
  --env release-automation \
  < /secure/path/to/nerv-ops-private-key.pem
```

After the environment secret is confirmed and release authentication is validated, delete the repository-level secret with the same name and revoke the superseded App key.

### 3. Bind npm trusted publishing

In the npm settings for `nervous-system`, configure the GitHub Actions trusted publisher with exactly:
...
```bash
gh secret list --repo 10ego/NERVous-system
gh secret list --repo 10ego/NERVous-system --env release-automation
gh secret list --repo 10ego/NERVous-system --env npm-publish
gh api repos/10ego/NERVous-system/environments/release-automation
gh api repos/10ego/NERVous-system/environments/npm-publish
npm view nervous-system version dist-tags --json
```
...
```bash
gh variable delete RELEASE_AUTOMATION_ENABLED --repo 10ego/NERVous-system
```
