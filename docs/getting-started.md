...
If another run is already streaming, the command waits for it to become idle before enabling any NERVous capability or dispatching the bundled workflow prompt. It then verifies Pi's native auto-retry setting. When retry is disabled or unreadable, no workflow starts and no activation marker is written; enable retry in Pi settings and rerun `/nervous`. Activation adds only the NERVous tools that were present in the operator's initial active-tool selection; excluded components remain blocked.
