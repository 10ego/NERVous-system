...
describe("npm release package policy", () => {
	...
	test("accepts the current lifecycle-script-disabled package", () => {
		...
		assert.equal(current.paths.length, 99);
	});
	...
	test("rejects unsafe, unexpected, executable, and duplicate package paths", () => {
		...
		for (const invalid of [
			...
			{ path: "docs/payload.sh", size: 1, mode: 0o644 },
			{ path: "docs/executable.md", size: 1, mode: 0o755 },
		]) {
	...
	test("never runs package lifecycle scripts during inspection", () => {
		const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nervous-package-policy-"));
		try {
