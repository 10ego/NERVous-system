...
- **`/nervous:config`** — open a TUI menu or show/set persistent suite enablement, drain/risk defaults, and shared NERVous model defaults. `enabled=false` reloads with the suite disabled; the config command remains so `enabled=true` can turn it back on.
...
In TUI mode, empty `/nervous:config` opens a settings-style menu for suite enablement, drain mode, risk gate, drain policy, and model defaults. Changing suite enablement reloads Pi: `enabled=false` unloads NERVous tools, workflow commands, skills, and prompts while retaining `/nervous:config`; `enabled=true` reloads the complete suite. When pi's model registry is available, model rows open a searchable picker. Selected values apply immediately; Esc closes the menu. Use `/nervous:config show` for markdown output. Outside TUI, empty `/nervous:config` falls back to markdown.
...
- Stored in `~/.pi/agent/nervous.json`, with trusted project overlay from `<repo>/.pi/nervous.json`. `enabled` defaults to `true` when omitted.
