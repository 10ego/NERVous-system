...
export const MAX_SIDECAR_FILES = 8_192;
export const MAX_SIDECAR_STORAGE_BYTES = 128 * 1024 * 1024;
...
export interface LionIoCounters {
	...
	sidecar_lock_acquisitions: number;
	sidecar_capacity_scans: number;
	sidecar_capacity_entries: number;
...
export function emptyLionIoCounters(): LionIoCounters {
	return {
		...
		sidecar_reads: 0, sidecar_writes: 0, sidecar_backups: 0, sidecar_lock_acquisitions: 0,
		sidecar_capacity_scans: 0, sidecar_capacity_entries: 0, sidecar_bytes_read: 0, sidecar_bytes_written: 0,
	};
}
...
interface Candidate { envelope: ProgressEnvelope; raw: Buffer; kind: "primary" | "backup" }
interface LatestResult { candidate?: Candidate; warnings: string[]; invalid: Array<{ filePath: string }> }

export interface ProgressFlushRequest {
	ref: Pick<LionRun, "id" | "incarnation_id">;
	progress: LionProgressSnapshot;
}

export type ProgressFlushOutcome =
	| { ok: true; progress: LionProgressSnapshot | undefined }
...
export class ProgressSnapshotStore {
	...
	private readonly limits = { maxFiles: MAX_SIDECAR_FILES, maxBytes: MAX_SIDECAR_STORAGE_BYTES };
	...
	async flush(ref: Pick<LionRun, "id" | "incarnation_id">, progress: LionProgressSnapshot): Promise<LionProgressSnapshot | undefined> {
		...
		this.counters.sidecar_lock_acquisitions++;
		return this.withLock(this.lockPath, () => this.flushUnlocked(ref, progress));
	}

	async flushBatch(requests: ProgressFlushRequest[]): Promise<ProgressFlushOutcome[]> {
		if (!requests.length) return [];
		this.counters.sidecar_lock_acquisitions++;
		return this.withLock(this.lockPath, async () => {
			const outcomes: ProgressFlushOutcome[] = [];
			for (const { ref, progress } of requests) {
				try {
					if (!ref.incarnation_id) outcomes.push({ ok: true, progress: undefined });
					else if (!validateProgress(progress)) throw new ProgressSidecarError(`progress snapshot exceeds the ${MAX_PROGRESS_SNAPSHOT_BYTES}-byte bounded schema`);
					else outcomes.push({ ok: true, progress: await this.flushUnlocked(ref, progress) });
				} catch (error) { outcomes.push({ ok: false, error }); }
			}
			return outcomes;
		});
	}

	private async flushUnlocked(ref: Pick<LionRun, "id" | "incarnation_id">, progress: LionProgressSnapshot): Promise<LionProgressSnapshot | undefined> {
	...
	async openUnlocked(ref: ExactRef): Promise<void> {
		await this.ensureRootUnlocked(true);
		...
		await this.assertAdmissionCapacityUnlocked();
	...
	async overlayUnlocked(ledger: LionLedger): Promise<string[]> {
		...
		for (const ref of ledger.activeExactRefs()) {
			const latest = await this.readLatestExactUnlocked(ref);
			for (const warning of latest.warnings) pushWarning(warnings, warning);
			if (latest.candidate?.envelope.progress) ledger.foldProgressIfCurrent(ref.id, ref.incarnation_id, latest.candidate.envelope.progress);
		}
		return warnings;
	}
	...
	async cleanupUnlocked(ledger: LionLedger, canonicalCertain = true): Promise<string[]> {
		...
		for (const name of names) {
			...
			try { candidate = await this.readCandidate(filePath, name.endsWith(".json") ? "primary" : "backup"); }
				...
				await this.quarantineInvalidUnlocked([{ filePath }]);
				continue;
			}
	...
	private async ensureRootUnlocked(recoverAbandonedTemps = false): Promise<void> {
		await this.assertSafeParentUnlocked();
		...
		if (recoverAbandonedTemps) await this.removeAbandonedTempsUnlocked();
	}
	...
	private async assertSafeParentUnlocked(): Promise<void> {
		const parent = path.dirname(this.root);
		const stat = await fs.lstat(parent);
		if (stat.isSymbolicLink() || !stat.isDirectory() || (stat.mode & 0o022) !== 0) {
			throw new ProgressSidecarError(`lion progress sidecar rejects unsafe parent ${parent}`);
		}
	}
	...
	private async assertSafeRootUnlocked(): Promise<boolean> {
		...
		try {
			...
			if (stat.isSymbolicLink() || !stat.isDirectory() || (stat.mode & 0o022) !== 0) throw new ProgressSidecarError(`lion progress sidecar rejects unsafe root ${this.root}`);
			return true;
		} catch (error) {
	...
	private async readLatestExactUnlocked(ref: ExactRef): Promise<LatestResult> {
		...
		const invalid: Array<{ filePath: string }> = [];
		const warnings: string[] = [];
	...
	private async readCandidate(filePath: string, kind: "primary" | "backup", ref?: ExactRef): Promise<Candidate | undefined> {
		...
		return { envelope: parseEnvelope(raw, this.namespace, ref), raw, kind };
	}
	...
	private async quarantineInvalidUnlocked(entries: Array<{ filePath: string }>): Promise<void> {
		if (!entries.length) return;
		await this.ensureRootUnlocked();
		await fs.mkdir(this.quarantineRoot, { recursive: true, mode: 0o700 });
		const quarantine = await fs.lstat(this.quarantineRoot);
		if (quarantine.isSymbolicLink() || !quarantine.isDirectory() || (quarantine.mode & 0o022) !== 0) throw new ProgressSidecarError(`lion progress sidecar rejects unsafe quarantine ${this.quarantineRoot}`);
	...
	private async assertAdmissionCapacityUnlocked(): Promise<void> {
		const names = await fs.readdir(this.root);
		this.counters.sidecar_capacity_scans++;
		this.counters.sidecar_capacity_entries += names.length;
		let files = 0;
		let bytes = 0;
		for (const name of names) {
			try {
				...
				files++;
				bytes += stat.size;
			} catch { /* an unavailable entry cannot make admission less conservative */ }
		}
		// Reserve both artifacts at their maximum bounded size when opening an
		// authority. Replacements can then remain constant-I/O under the lock.
		if (files + 2 > this.limits.maxFiles || bytes + (MAX_PROGRESS_ENVELOPE_BYTES * 2) > this.limits.maxBytes) {
			throw new ProgressSidecarError("progress sidecar capacity reached; cleanup is required before admitting another execution");
		}
	}
}
