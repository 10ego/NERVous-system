...
- `runner_mode="rpc"` (or `LION_RUNNER=rpc`) opts into live mid-run steering through pi RPC; the default `json` runner rejects running steering. RPC setup through final I/O shares one deadline, interrupted start adopts any spawned child, and all delayed steering writes compare the immutable run incarnation. If bounded stop leaves the attached child alive, the call may return explicit nonterminal `cleanup_pending`; process-local supervision retains the exact owner until observed exit and late fenced finalization. Persisted PID metadata never permits restart reattachment or signaling.
