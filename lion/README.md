...
If bounded stop cleanup leaves the attached child alive, the RPC runner returns the explicit nonterminal `cleanup_pending` outcome only after registering a process-local supervisor with the exact namespace/run/incarnation/owner capability, child observation, temporary resources, and terminal intent. Registration failure preserves the foreground wait. A successful handoff keeps the durable run running and retains cancellation ownership until confirmed child exit; the supervisor then retries cleanup and exact-incarnation finalization before releasing ownership. The registry is intentionally not durable: a restart never reattaches to or signals persisted PID/PGID metadata.
