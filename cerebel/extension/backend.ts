...
export interface CerebelLocation {
	cerebelPath: string;
	dir: string;
}
...
function isPidAlive(pid: number): boolean {
	...
	catch (err) { const code = (err as NodeJS.ErrnoException).code; return code !== "ESRCH" && code !== "EINVAL"; }
}
...
export async function withLock<T>(lockPath: string, fn: () => Promise<T>): Promise<T> {
	...
	for (;;) {
		try {
			...
			await handle.close();
			try { return await fn(); }
			finally { try { await fs.unlink(lockPath); } catch { /* ignore */ } }
		} catch (err) {
			const code = (err as NodeJS.ErrnoException).code;
			if (code !== "EEXIST") throw err;
			attempts++;
			if (attempts % 20 === 0 && await isLockStale(lockPath)) { try { await fs.unlink(lockPath); } catch { /* raced */ } continue; }
			if (attempts >= LOCK_MAX_ATTEMPTS) throw new Error(`cerebel: timed out acquiring lock ${lockPath}`);
			await sleep(LOCK_DELAY_MS);
		}
	}
}

export interface LoadResult { ledger: CerebelLedger; warnings: string[]; fresh: boolean }

export class FileBackend {
	readonly location: CerebelLocation;
	private readonly lockPath: string;
	private readonly tmpPath: string;
	private readonly bakPath: string;
	constructor(location: CerebelLocation) {
		this.location = location;
		this.lockPath = `${location.cerebelPath}.lock`;
		this.tmpPath = `${location.cerebelPath}.tmp`;
	...
	async load(): Promise<LoadResult> {
		...
		try { raw = await fs.readFile(this.location.cerebelPath, "utf8"); }
			...
			const code = (err as NodeJS.ErrnoException).code;
			if (code === "ENOENT") return { ledger: new CerebelLedger(), warnings: [], fresh: true };
			throw err;
		}
...
export class CerebelStore {
	readonly backend: FileBackend;
	constructor(backend: FileBackend) { this.backend = backend; }
	static fromCwd(cwd: string): CerebelStore { return new CerebelStore(new FileBackend(resolveCerebelLocation(cwd))); }
	async query<T>(fn: (ledger: CerebelLedger) => T): Promise<{ result: T; warnings: string[] }> {
