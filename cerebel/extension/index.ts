	...
	try {
		...
		return {
			...
			async finishRun(runId, result) {
				...
				try {
					const outcome = ("finishRun" in lionStore && typeof lionStore.finishRun === "function")
						? (await lionStore.finishRun(runId, owner.incarnationId, result)).result
						: (await lionStore.mutate((ledger) => ledger.finishIfCurrent(runId, owner.incarnationId, result))).result;
